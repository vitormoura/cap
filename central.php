...
else if( !empty( $_POST['LSconta']) && !empty( $_POST['LSforma']) && !empty( $_POST['TXTdata']) && isset( $_POST['select']) && $_POST['HDtotal'] > 0 && isset( $_POST['RDbatch'] ) && $_SESSION['nivel'] >= 2 )
{
	//Gravando todos os checkboxes selecionados:
	$itens = $_POST['select'];
	$valor = $_POST['HDtotal'];
	
	//Testamos se devemos tratar os documentos como um bloco ou nуo
	if( $_POST['RDbatch'] == 'block' ) {
	...
	//Aqui tratamos cada item individualmente, efetuando um batch da baixa ( Estava cansado de fazer aquilo manualmente )
	else if( $_POST['RDbatch'] == 'single' ) 
	{
		//Capturando o array com os valores liquidos :
		$valores = $_POST['liq'];
							
		//Inserindo cada item ...			
		foreach ( $itens as $item ) {
			
			//Valor deste item em particular...
			$valor 		= $valores[$item];
			
			$insertSql  = sprintsql( "INSERT INTO movimentacao ( codForma, codConta, dtMov, numero, nPagamentos, valor, tipo ) VALUES ( %d, %d, %t, %s, %d, $valor, %s )",
							 $_POST['LSforma'], $_POST['LSconta'], $_POST['TXTdata'], $_POST['TXTnum'], 1, 'd');
	
			//Gravando a movimentaчуo:
			$cod  = dbQuery( $insertSql );
				
			
			//Preparando uma query para gravar o item individual...
			$temp = "UPDATE pagamento SET pagamento.codMov = $cod, situacao = 'p' WHERE codDoc = $item";
			
			//Atualizando o pagamento no banco de dados:
			dbQuery( $temp );
		}
	}
	else 
	{
		die( 'Aconteceu um erro imprevisto, o tipo de empacotamento esta incorreto.');
