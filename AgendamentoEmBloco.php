	...
	!empty( $_POST['TXTvenc']) && !empty( $_POST['TXTval']) && !empty( $_POST['LSdoc'])   && !empty( $_POST['RDfiscal']) && !empty($_POST['LSplano']))
{
	...
	$user       = 1;		         		//Codigo do usuario, sempre admin
	$dtGravacao = mktime(0,0,0,01,01,2006); //Iniciando o ano...
	$tipo       = $_POST['LSdoc'];			//Tipo de documento
	$nota       = 0;		         		//Comentario
	$loja       = $_POST['LSloja'];  		//Codigo da loja
	$forn       = $_POST['LSforn'];		    // codigo do fornecedor
	$emiss      = $_POST['TXTemis'];		//date("d-m-Y");
	$valor      = $_POST['TXTval'];	        //Valor
	$cont       = $_POST['RDfiscal'];		//Contabil ?
	$plano		= $_POST['LSplano'];		//Plano de contas...
	list( $dia, $mes, $ano ) = explode( '-', $_POST['TXTvenc']  ); //capturando dia, mes e ano
	$num        = $_POST['TXTnum'];			//numero do documento...
...
<title>Agendamento em Bloco</title>
...
<table width="485" border="0" cellspacing="0" class="topoGlobal">
  ...
  <tr class="topoBase">
    <td height="33" colspan="3"><span class="titulo">Agendamento em Bloco</span></td>
  </tr>
</table>
