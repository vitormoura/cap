...
//Valores default para cada item enviado: ( Em caso de terem sido deixados em branco )
	...
	$result = sprintsql("SELECT tpdoc.tipo, colaborador.fantasia AS forn, loja.fantasia AS loja, documento.numero, documento.emissao, documento.valor,
						 ...
						 AND documento.vencimento BETWEEN %t AND %t AND documento.codTp LIKE %s AND documento.contabil LIKE %s AND pagamento.codPlano LIKE %s AND pagamento.situacao LIKE %s ORDER BY documento.fornecedor, documento.loja", 
						 $loja, $forn, $user, $num, $agen1, $agen2, $emis1, $emis2, $venc1, $venc2, $doc, $fisc, $plano, $stat
						 );
    ...
    <td height="61" colspan="2" align="center" valign="top"><table width="906" border="0" cellspacing="0">
		...
		if( $dados = rsFetch( $rs ) ) { 
			...
			while( $dados ) {
				...
				//atualizando o novo indice do subtotal
				$subt = $dados[$subtotalizador];
				
				//zerando os subtotais...
				$subTotalBruto = 0.0;
				$subTotalAcres = 0.0;
				$subTotalDesc  = 0.0;
				$subTotalDed   = 0.0;
				...
				do 
				{
					...
					$subTotalBruto += $dados['valor'];
					$subTotalAcres += $dados['acrescimos'];
					$subTotalDesc  += $dados['descontos'];
					$subTotalDed   += $dados['abatimentos'];
...
DADOS;
				...
				while( $dados[$subtotalizador] == $subt );
				
				
				//escrevendo a subtotalizacao...
				echo <<<SUBTOTAL
...
SUBTOTAL;
  	      ...
  	      echo '<td align="right">'.restaurar( $subTotalBruto, 'valor' ).'</td>';
          echo '<td align="right">'.restaurar( $subTotalDesc, 'valor' ).'</td>';
          echo '<td align="right">'.restaurar( $subTotalAcres, 'valor' ).'</td>';
          echo '<td align="right">'.restaurar( $subTotalDed, 'valor' ).'</td>';
          echo '<td align="right">'.restaurar( ( $subTotalBruto + $subTotalAcres ) - ( $subTotalDesc + $subTotalDed ), 'valor' ).'</td>';
