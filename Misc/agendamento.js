...
{ 
	...
	/*
	//Testamos se o vencimento informado é menor que hoje, considerando-o vencido
	if( n2 < n3 )
	{
		if( !confirm('Confirma o agendamento de um documento já vencido ?') )
			return false;
	}
	//*/
...
function apurarDesconto() {
	
	//Capturamos os valores dos campos e os interpretamos como valores numéricos :
	var vlBase = parseFloat(document.getElementById('TXTbase').value.replace('.','').replace(',','.'));
	var vlDesc = parseFloat(document.getElementById('TXTdesc').value.replace('.','').replace(',','.'));
	
	//testamos se ambos os valores são válidos :
	if( vlBase && vlDesc ) 
	{
		//Criamos um novo objeto número, formatado com duas casas decimais
		resultado = new Number( ( vlDesc * 100 ) / vlBase );
	
		//Exibimos o resultado no campo destino
		document.getElementById('descDisplay').firstChild.nodeValue = resultado.toFixed(2)+"%";
	}
	else
		//Se houver erros com os valores, limpamos o valor do desconto
		document.getElementById('descDisplay').firstChild.nodeValue = "";
